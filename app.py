...
def display_other_feature_ui2(match_id):
    st.markdown("""
    <style>
    ...
    main_match_id = "".join(filter(str.isdigit, match_id))
    if not main_match_id:
        results_container.warning("⚠️ ID de partido no válido proporcionado.")
        st.stop()
...
URL_NOWGOAL_MAIN = "https://live20.nowgoal25.com/"

def parse_main_page_matches(html_content):
    """Parsea el HTML de la página principal para obtener la lista de partidos."""
    ...
    for row in match_rows:
        ...
        upcoming_matches.append({
            ...
            "Equipo Local": home_team_tag.text.strip() if home_team_tag else "N/A",
            "Equipo Visitante": away_team_tag.text.strip() if away_team_tag else "N/A",
        })
    upcoming_matches.sort(key=lambda x: (x['Fecha'], x['Hora']))
    return upcoming_matches

async def fetch_main_page_html():
    """Usa Playwright para cargar la página y obtener el HTML."""
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page()
        try:
            await page.goto(URL_NOWGOAL_MAIN, wait_until="networkidle", timeout=30000)
            await page.wait_for_selector('tr[id^="tr1_"]', timeout=20000)
...
def get_upcoming_matches():
    """Función cacheada para obtener y parsear los partidos."""
    try:
        html_content = asyncio.run(fetch_main_page_html())
...
def render_main_page():
    """Muestra la página principal con la lista de partidos."""
    st.title("⚽ Visor de Partidos de Fútbol")
    st.markdown("Lista de partidos próximos. Selecciona un partido de la tabla para ver el análisis detallado.")

    if st.button("Refrescar Lista"):
        get_upcoming_matches.clear()
        st.success("Lista de partidos refrescada.")
        st.rerun()

    matches = get_upcoming_matches()
    if not matches:
        st.warning("No se encontraron partidos. Intenta refrescar en unos momentos.")
        return
    ...
    st.subheader("Analizar un partido por ID")
    selected_id = st.text_input("Copia y pega el ID de un partido de la tabla de arriba:")
    if st.button("Analizar Partido"):
        if selected_id and selected_id.isdigit():
            st.session_state.selected_match_id = selected_id
            st.rerun()
        else:
            st.warning("Por favor, ingresa un ID numérico válido.")

def render_detailed_view(match_id):
    """Renderiza la vista de detalle."""
    if st.button("⬅️ Volver a la lista"):
        st.session_state.selected_match_id = None
        st.rerun()

    # La función original de estudio.py ahora se usa para renderizar la página
    display_other_feature_ui2(match_id)
...
if st.session_state.selected_match_id:
    render_detailed_view(st.session_state.selected_match_id)
else:
    render_main_page()
